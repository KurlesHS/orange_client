...
#define declare_signal(name, signature) \
public: \
template<typename T> \
size_t connect_ ## name(T &&slot) \
{  return m_ ## name.connect(std::forward<T>(slot)); } \
bool disconnect_ ## name(const size_t &connection_id) \
{  return m_ ## name.disconnect(connection_id); } \
void disconnect_ ## name() \
{ m_ ## name.clear(); } \
protected: \
template<typename ...Args> \
void emit_ ## name(Args && ...args) \
{ m_ ## name.emit(std::forward<Args>(args)...); } \
private: \
Simple::Signal<signature> m_ ## name
